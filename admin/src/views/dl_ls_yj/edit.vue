<template>
    ...
    <el-input v-model="formData.name" clearable placeholder="请输入名称" />
                ...
                <el-form-item label="最大投注" prop="top_tz">
    <el-input v-model="formData.top_tz" clearable placeholder="请输入最大投注" />
</el-form-item>
                <el-form-item label="最小投注" prop="low_tz">
    <el-input v-model="formData.low_tz" clearable placeholder="请输入最小投注" />
</el-form-item>
                <el-form-item label="最低活跃用户" prop="low_user">
    <el-input v-model="formData.low_user" clearable placeholder="请输入最低活跃用户" />
</el-form-item>
...
<script lang="ts" setup name="dlLsYjEdit">
import type { FormInstance } from 'element-plus'
import Popup from '@/components/popup/index.vue'
import { apiDlLsYjAdd, apiDlLsYjEdit, apiDlLsYjDetail } from '@/api/dl_ls_yj'
...
const popupTitle = computed(() => {
    return mode.value == 'edit' ? '编辑代理流水佣金策略' : '新增代理流水佣金策略'
})
...
const getDetail = async (row: Record<string, any>) => {
    const data = await apiDlLsYjDetail({
        id: row.id
    })
    setFormData(data)
}
...
const handleSubmit = async () => {
    ...
    const data = { ...formData,  }
    mode.value == 'edit' 
        ? await apiDlLsYjEdit(data) 
        : await apiDlLsYjAdd(data)
